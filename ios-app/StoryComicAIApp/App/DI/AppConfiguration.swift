...
struct AppConfiguration {
    ...
    static func resolve(processInfo: ProcessInfo = .processInfo) -> AppConfiguration {
        ...
        let mockFlag = environment["STORYCOMICAI_USE_MOCK_SERVICES"].flatMap { Bool($0) } ?? false
        let directLaunch = environment["STORYCOMICAI_SKIP_ENTRY_FLOW"].flatMap { Bool($0) } ?? false
